...
import 'package:test/common/third_party/printing/lib/src/interface.dart';
...
class PdfPrintAction extends StatelessWidget {
  ...
  Future<void> _print(
    ...
    // print("dkh build $build");
    // print("dkh printers ${printers[0]}");
  ...
  Future<void> printDirectPdf(LayoutCallback build, {String? name}) async {
    try {
      List<Printer> printers = await Printing.listPrinters();
      // print("dkh build $build");
      // print("dkh printers ${printers[0]}");
      ...
      if (printer == null && printers.isNotEmpty) {
        printer = printers[0];
      }
      if (printer != null) {
        await PrintingPlatform.instance.layoutPdf(
          printer,
          build,
          jobName,
          PdfPageFormat.roll80,
      ...
      assert(() {
        collector = () sync* {
          yield StringProperty('PageFormat', PdfPageFormat.roll80.toString());
        };
        return true;
      }());
